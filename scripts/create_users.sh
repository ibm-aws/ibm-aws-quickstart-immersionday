...
function create_users() {
  ...
  if [ "$code" = "200" ]; then
    ...
    curl -k -X POST -H "Authorization: Bearer $token" -H "cache-control: no-cache" -d "{\"user_name\":\"datascientist$ram\",\"password\":\"password\",\"displayName\":\"datascientist$ram\",\"permissions\":[\"access_catalog\",\"create_project\",\"create_space\"],\"user_roles\":[\"wkc_data_scientist_role\"],\"email\":\"datascientist@test.com\"}" "https://$route/icp4d-api/v1/users" -H "Content-Type: application/json"
